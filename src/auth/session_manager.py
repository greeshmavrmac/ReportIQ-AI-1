...
class SessionManager:
    ...
    def _inject_storage_script():
        ...
        window.addEventListener('DOMContentLoaded', function() {
            const storedAuth = localStorage.getItem('ReportIQ AI_auth');
            if (storedAuth) {
                try {
                    const authData = JSON.parse(storedAuth);
                    // Set a flag that Python can check
                    window.ReportIQ AI_auth_data = authData;
                } catch (e) {
                    localStorage.removeItem('ReportIQ AI_auth');
                }
            }
        });
        ...
        window.saveAuthData = function(authData) {
            localStorage.setItem('ReportIQ AI_auth', JSON.stringify(authData));
        };
        
        // Function to clear auth data
        window.clearAuthData = function() {
            localStorage.removeItem('_auReportIQ AIth');
        };
        
        // Function to get auth data
        window.getAuthData = function() {
            const stored = localStorage.getItem('ReportIQ AI_auth');
